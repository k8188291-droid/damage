    ...
    <Modal open title="編輯技能" onClose={onClose} width="max-w-2xl">
      <div className="space-y-4">
        <div className="grid grid-cols-4 gap-3">
          <div>
            <label className="block text-xs text-ef-ink-3 mb-1">技能名稱</label>
          ...
          <div>
            <label className="block text-xs text-ef-ink-3 mb-1">角色</label>
          ...
          <div>
            <label className="block text-xs text-ef-ink-3 mb-1">技能倍率 (%)</label>
            <input type="number" value={d.skillMultiplier || ''} onChange={e => p({ skillMultiplier: Number(e.target.value) || 0 })}
              className="w-full bg-ef-input border border-ef-line rounded-lg px-3 py-2 text-sm text-ef-ink focus:outline-none focus:border-ef-gold" placeholder="100" />
          </div>
          <div>
        ...
        <div>
          <div className="flex items-center justify-between mb-2">
            <div className="flex items-center gap-3">
              <label className="text-xs text-ef-ink-3 font-medium">啟用的 Buff</label>
              <div className="flex items-center bg-ef-card rounded-lg border border-ef-line overflow-hidden">
                <button onClick={() => setGroupBy('group')}
                  className={`px-2 py-1 text-xs font-medium cursor-pointer transition-colors ${groupBy === 'group' ? 'bg-ef-gold text-white' : 'text-ef-ink-3 hover:text-ef-ink'}`}>
                  依群組
                </button>
                <button onClick={() => setGroupBy('zone')}
                  className={`px-2 py-1 text-xs font-medium cursor-pointer transition-colors ${groupBy === 'zone' ? 'bg-ef-gold text-white' : 'text-ef-ink-3 hover:text-ef-ink'}`}>
                  依分區
                </button>
              </div>
            </div>
            <div className="flex gap-3">
              <button onClick={() => p({ enabledBuffIds: buffs.map(b => b.id) })} className="text-xs text-ef-gold hover:text-ef-gold/80 cursor-pointer">全選</button>
              <button onClick={() => p({ enabledBuffIds: [] })} className="text-xs text-ef-ink-3 hover:text-ef-ink cursor-pointer">全不選</button>
            </div>
          </div>
          {buffs.length === 0 ? (
            <p className="text-xs text-ef-ink-4">尚未設定 Buff</p>
          ) : (
...
function SortableSkillCard({ skill, char, onClick, onCopy, onRemove }: {
  ...
  return (
    <div ref={setNodeRef} style={style}
      className="bg-ef-card border border-ef-line rounded-xl px-3 py-2 cursor-pointer hover:border-ef-gold/50 transition-colors group"
      onClick={onClick}>
      <div className="flex items-center gap-2">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between">
            <span className="text-sm font-medium text-ef-ink truncate">{skill.name}</span>
            <span className="text-xs text-ef-gold font-mono shrink-0 ml-2">{skill.skillMultiplier}%</span>
          </div>
          <div className="text-[10px] text-ef-ink-3 mt-0.5">
            {char ? char.name : '未指定'}
          </div>
        </div>
        <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity shrink-0">
          <button onClick={e => { e.stopPropagation(); onCopy(); }} className="text-ef-ink-4 hover:text-ef-gold text-xs cursor-pointer" title="複製">⧉</button>
          <button onClick={e => { e.stopPropagation(); onRemove(); }} className="text-ef-ink-4 hover:text-red-400 text-xs cursor-pointer" title="刪除">✕</button>
        </div>
      </div>
    </div>
  );
...
function SkillCardOverlay({ skill, char }: { skill: Skill; char: Character | undefined }) {
  return (
    <div className="bg-ef-card border border-ef-gold rounded-xl px-3 py-2 shadow-xl shadow-ef-gold/20 cursor-grabbing">
      <div className="text-sm font-medium text-ef-ink">{skill.name}</div>
      <div className="text-xs text-ef-ink-3">{char ? char.name : '未指定'} · {skill.skillMultiplier}%</div>
    </div>
  );
}
...
function DroppableSkillArea({ groupId, children, isEmpty }: {
  ...
  return (
    <div ref={setNodeRef}
      ...
      {children}
      {isEmpty && (
        <div className="w-full text-xs text-ef-ink-4 text-center py-2">
    ...
    <section>
      <div className="flex gap-2 mb-3 flex-wrap">
        <button onClick={addSkillGroup}
          className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-[11px] font-medium bg-ef-card hover:bg-ef-card-hover border border-ef-line text-ef-ink-2 cursor-pointer transition-colors">
          + 群組
        </button>
      ...
      {skills.length === 0 && skillGroups.length === 0 ? (
        <p className="text-ef-ink-4 text-xs text-center py-3">尚未新增技能</p>
      ) : (
        <DndContext sensors={sensors} collisionDetection={closestCenter}
          ...
          {(ungroupedSkills.length > 0 || skillGroups.length > 0) && (
            <div className="mb-2">
              {skillGroups.length > 0 && <div className="text-[10px] text-ef-ink-4 mb-1">未分組</div>}
            ...
            return (
              <div key={g.id} className="mb-2 border border-ef-line rounded-xl overflow-hidden">
                <div className="flex items-center gap-2 px-3 py-1.5 bg-black/5" style={{ borderLeft: `3px solid ${g.color}` }}>
                  ...
                  />
                  <button onClick={() => removeSkillGroup(g.id)} className="text-ef-ink-3 hover:text-red-400 text-xs cursor-pointer shrink-0">✕</button>
                </div>
